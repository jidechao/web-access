...
以下步骤适用于 `PowerShell`（建议 PowerShell 7+）：
...
# 2) 进入目录
Set-Location $env:USERPROFILE\.claude\skills\web-access
```

```powershell
# 3) 打开 Chrome 并开启 remote debugging
# 在地址栏打开：chrome://inspect/#remote-debugging
...
```powershell
# 5) 启动 CDP proxy（后台）
Start-Process -FilePath node -ArgumentList ".\scripts\cdp-proxy.mjs" -WindowStyle Hidden
```

```powershell
# 6) 用 Invoke-RestMethod 调用 API
# 新建 tab
$new = Invoke-RestMethod -Uri "http://localhost:3456/new?url=https://example.com"
$target = $new.targetId

# 获取页面标题
Invoke-RestMethod -Method Post -Uri "http://localhost:3456/eval?target=$target" -Body "document.title"

# 截图到本地（会写入当前目录下 tmp）
New-Item -ItemType Directory -Force -Path .\tmp | Out-Null
Invoke-RestMethod -Uri "http://localhost:3456/screenshot?target=$target&file=./tmp/shot.png"

# 关闭 tab
Invoke-RestMethod -Uri "http://localhost:3456/close?target=$target"
```
...
```powershell
# 启动（Windows）
Start-Process -FilePath node -ArgumentList "$env:USERPROFILE\.claude\skills\web-access\scripts\cdp-proxy.mjs" -WindowStyle Hidden
